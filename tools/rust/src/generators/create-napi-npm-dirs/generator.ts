...
export default async function (tree: Tree, options: CreateNapiNpmDirsGeneratorSchema) {
...
function addNpmFiles(project: ProjectConfiguration) {
    ...
    runProcess(command, "-c", joinPathFragments(project.root, "package.json"), "-t", project.root);
}
