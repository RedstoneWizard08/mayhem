...
export const processProjectGraph: ProjectGraphProcessor = (
    ...
    const { success, output } = cargoCommandSync(false, "metadata --format-version=1", {
        stdio: "pipe",
    });
    if (!success) {
        return graph;
...
function addExplicitDependency(pkg: Package, builder: ProjectGraphBuilder, depName: string) {
    const target =
    ...
    builder.addStaticDependency(pkg.name, depName, target.replace(`${workspaceRootClean}/`, ""));
}
